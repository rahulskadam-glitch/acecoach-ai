...
With both services running, use `npm run verify:runtime` to check the connected Supabase schema, coaching-persistence mode, video-storage allowlist, and analysis health endpoint. Open `http://localhost:3000/version` and confirm version **6.0.0**. Migration `supabase/022_simple_athlete_journey_v600.sql` remains the preferred production schema; environments awaiting that migration automatically use report-backed coaching persistence without losing the existing coaching playbook.
