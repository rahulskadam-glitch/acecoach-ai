  ...
  return (
    <div className="mx-auto max-w-md py-2 sm:py-8">
      <section className="ath-card relative overflow-hidden p-6 sm:p-8">
        <div className="absolute inset-x-0 top-0 h-1.5 bg-[linear-gradient(90deg,#40916c_0%,#c9a227_52%,#d7e022_100%)]" />
        <div className="text-center">
          <p className="text-xs font-bold uppercase tracking-[0.17em] text-[#40916c]">Athlentra {sportName}</p>
        ...
        <form onSubmit={submitEmail} className="space-y-4">
          {mode === "signup" ? <label className="block text-sm font-medium text-slate-700">Full name<div className="mt-2 flex items-center gap-3 rounded-xl border border-[#d6e2cf] bg-white px-4 focus-within:border-[#40916c] focus-within:ring-2 focus-within:ring-[#c9a227]/30"><input value={name} onChange={(event) => setName(event.target.value)} className="min-h-12 w-full bg-transparent outline-none" autoComplete="name" required placeholder="Your name" /></div></label> : null}
          <label className="block text-sm font-medium text-slate-700">Email<div className="mt-2 flex items-center gap-3 rounded-xl border border-[#d6e2cf] bg-white px-4 focus-within:border-[#40916c] focus-within:ring-2 focus-within:ring-[#c9a227]/30"><Mail className="h-4 w-4 text-slate-400" aria-hidden="true" /><input value={email} onChange={(event) => setEmail(event.target.value)} type="email" className="min-h-12 w-full bg-transparent outline-none" autoComplete="email" required placeholder="you@example.com" /></div></label>
          <label className="block text-sm font-medium text-slate-700">Password<div className="mt-2 flex items-center gap-3 rounded-xl border border-[#d6e2cf] bg-white px-4 focus-within:border-[#40916c] focus-within:ring-2 focus-within:ring-[#c9a227]/30"><LockKeyhole className="h-4 w-4 text-slate-400" aria-hidden="true" /><input value={password} onChange={(event) => setPassword(event.target.value)} type={showPassword ? "text" : "password"} minLength={8} maxLength={128} className="min-h-12 w-full bg-transparent outline-none" autoComplete={mode === "signin" ? "current-password" : "new-password"} required placeholder="At least 8 characters" /><button type="button" onClick={() => setShowPassword((value) => !value)} className="rounded-lg p-2 text-slate-400 hover:text-slate-800" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></label>
        ...
        {mode === "signin" ? <Link href="/forgot-password" className="mt-5 block text-center text-sm font-medium text-[#40916c] hover:text-[#1b4332]">Forgot password?</Link> : null}
