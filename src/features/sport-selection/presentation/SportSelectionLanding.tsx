    ...
    <main className="min-h-screen overflow-hidden bg-[#071B2D] text-white">
      <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
        ...
        <div className="flex items-center gap-2">
          <a href="/auth" className="min-h-11 rounded-full px-4 py-3 text-sm font-semibold text-white/75 hover:text-white">Sign in</a>
          <button type="button" onClick={startTennis} disabled={selected} className="min-h-11 rounded-full bg-[#D8FF52] px-5 text-sm font-bold text-[#071B2D] hover:bg-white disabled:opacity-60">Start</button>
        </div>
      </header>
      ...
      <section className="relative mx-auto grid min-h-[calc(100vh-84px)] max-w-7xl items-center gap-12 px-5 pb-16 pt-8 sm:px-8 lg:grid-cols-[1fr_0.9fr] lg:py-20">
        ...
        <div className="relative z-10 max-w-2xl">
          <p className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-2 text-xs font-bold uppercase tracking-[0.18em] text-[#D8FF52]"><Sparkles className="h-4 w-4" />Your tennis. Made visible.</p>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <button type="button" onClick={startTennis} disabled={selected} className="group inline-flex min-h-14 items-center justify-center gap-3 rounded-2xl bg-[#D8FF52] px-7 font-bold text-[#071B2D] shadow-2xl shadow-lime-300/10 hover:bg-white disabled:cursor-wait disabled:opacity-60">{selected ? "Opening…" : "Analyze a stroke"}<ArrowRight className="h-5 w-5 transition group-hover:translate-x-1" /></button>
          ...
          <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm text-slate-400">
            <span className="flex items-center gap-2"><Check className="h-4 w-4 text-[#D8FF52]" />No sensors</span>
            <span className="flex items-center gap-2"><Check className="h-4 w-4 text-[#D8FF52]" />One phone</span>
            <span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-[#D8FF52]" />Your video, your control</span>
          </div>
        </div>
          ...
          <div className="relative overflow-hidden rounded-[2.5rem] border border-white/15 bg-[#0C263C] p-3 shadow-2xl">
            <div className="relative aspect-[4/5] overflow-hidden rounded-[2rem] bg-[radial-gradient(circle_at_50%_15%,rgba(59,216,164,.35),transparent_34%),linear-gradient(180deg,#123852_0%,#081a2b_100%)] p-5">
              ...
              <svg viewBox="0 0 260 270" className="relative mx-auto mt-6 h-[55%] w-full" aria-hidden="true">
                <path d="M53 223 C67 165 92 122 126 105 C163 86 196 101 218 137" fill="none" stroke="#D8FF52" strokeWidth="4" strokeLinecap="round" strokeDasharray="7 10" />
                <circle cx="122" cy="70" r="19" fill="#8AD8FF" /><path d="M122 90 L117 153 M118 113 L77 139 M119 112 L167 91 M117 151 L82 218 M117 151 L157 217" fill="none" stroke="#F8FAFC" strokeWidth="9" strokeLinecap="round" /><path d="M167 91 L212 67" stroke="#D8FF52" strokeWidth="7" strokeLinecap="round" /><ellipse cx="222" cy="61" rx="15" ry="26" fill="none" stroke="#D8FF52" strokeWidth="5" transform="rotate(52 222 61)" />
              </svg>
              <div className="absolute inset-x-5 bottom-5 rounded-2xl border border-white/10 bg-[#071B2D]/90 p-4 backdrop-blur">
      ...
      <section id="how" className="bg-[#f3f6f2] px-5 py-16 text-slate-950 sm:px-8 sm:py-24">
        <div className="mx-auto max-w-6xl">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#08715b]">From court to clarity</p>
