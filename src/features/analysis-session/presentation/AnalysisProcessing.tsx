      ...
      <div className="mx-auto max-w-3xl">
        <div className="rounded-3xl border border-amber-200 bg-white p-7 shadow-xl shadow-slate-200/60 sm:p-10">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => confirmMovement(status.detectedAction)} disabled={confirming || !status.detectedAction} className="min-h-12 rounded-xl bg-[#1b4332] px-5 font-semibold text-white hover:bg-[#2d6a4f] disabled:opacity-50">Use {detectedLabel}</button>
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <section className="mt-10 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-slate-200 pt-6">
          ...
          {showManualRetry ? <div className="flex gap-3">{failed ? <a href={`/start?sport=${sport.id}`} className="inline-flex min-h-11 items-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">Upload another video</a> : null}<button type="button" onClick={retry} disabled={starting} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#1b4332] px-4 text-sm font-semibold text-white hover:bg-[#2d6a4f] disabled:opacity-50"><RefreshCw className="h-4 w-4" />Retry analysis</button></div> : <button type="button" onClick={() => void refresh()} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">Refresh status<ArrowRight className="h-4 w-4" /></button>}
        </div>
      </section>
