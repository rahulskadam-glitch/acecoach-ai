...
export default function AnalysisProcessing({ sessionId, fileName, sport, initialStatus, staticPreview = false }: { sessionId: string; fileName: string; sport: SportDefinition; initialStatus: Status; staticPreview?: boolean }) {
  ...
  const attemptRun = useCallback(async () => {
  ...
  useEffect(() => {
    ...
    const eligible = statusRef.current.status === "queued" || statusRef.current.status === "processing" || statusRef.current.status === "failed";
    if (!eligible) return;
    void attemptRun();
    const timer = window.setInterval(() => {
      if (statusRef.current.status === "completed" || statusRef.current.status === "failed") {
    ...
    // Runs once per mount by design: subsequent status changes are read via
    // statusRef inside the interval, not re-triggering effect setup.
    ...
    return (
      <div className="mx-auto max-w-3xl">
        <div className="rounded-3xl border border-amber-200 bg-white p-7 shadow-xl shadow-slate-200/60 sm:p-10">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-slate-950">You selected {selectedLabel}.<br />Athlentra detected {detectedLabel}.</h1>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => confirmMovement(status.detectedAction)} disabled={confirming || !status.detectedAction} className="min-h-12 rounded-xl bg-[#071b2d] px-5 font-semibold text-white hover:bg-[#0d2b42] disabled:opacity-50">Use {detectedLabel}</button>
  ...
  const stuck = !failed && autoRetryExhausted;
  const showManualRetry = failed || stuck;
  return (
    <div className="mx-auto max-w-4xl">
      <div className="text-center">
        ...
        <h1 className="mt-3 text-4xl font-semibold tracking-[-0.035em] text-slate-950">{failed ? "We could not complete the analysis" : stuck ? "This is taking longer than expected" : "Athlentra is reviewing your movement"}</h1>
        <p className="mx-auto mt-4 max-w-2xl text-base leading-8 text-slate-600">{failed ? "Your original video is safe. Review the reason below, then retry or upload a better clip." : stuck ? "Your original video is safe and the analysis is still queued. You can keep waiting or retry now." : "You may leave this page and return. The stages below reflect the last confirmed server state—not a fabricated countdown."}</p>
      </div>
      ...
      <section className="mt-10 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-slate-200 pt-6">
          ...
          {showManualRetry ? <div className="flex gap-3">{failed ? <a href={`/start?sport=${sport.id}`} className="inline-flex min-h-11 items-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">Upload another video</a> : null}<button type="button" onClick={retry} disabled={starting} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#071b2d] px-4 text-sm font-semibold text-white hover:bg-[#0d2b42] disabled:opacity-50"><RefreshCw className="h-4 w-4" />Retry analysis</button></div> : <button type="button" onClick={() => void refresh()} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">Refresh status<ArrowRight className="h-4 w-4" /></button>}
        </div>
      </section>
