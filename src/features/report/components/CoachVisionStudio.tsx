...
const STATUS_STYLE: Record<BiomechanicalLinkage["status"], { color: string; label: string; classes: string }> = {
  connected: { color: "#2fa87f", label: "connected", classes: "border-ath-green/25 bg-ath-green/10 text-ath-green" },
...
const VISUAL_STATUS = {
  good: { color: "#2fa87f", label: "Working", classes: "border-ath-green/25 bg-ath-green/10 text-ath-green" },
  ...
  reference: { color: "#5aa3d8", label: "Benchmark", classes: "border-ath-sky/40 bg-ath-sky/10 text-ath-navy" },
} as const;
      ...
      <div className="grid xl:grid-cols-[minmax(0,1.48fr)_minmax(360px,.72fr)]">
        <div className="border-b border-slate-200 p-4 sm:p-6 xl:border-b-0 xl:border-r">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={() => void toggle()} disabled={previewOnly} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#123049] px-5 font-semibold text-white hover:bg-[#1a4060] disabled:cursor-not-allowed disabled:opacity-50">{playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}{previewOnly ? "Preview frames" : playing ? "Pause" : "Play"}</button>
