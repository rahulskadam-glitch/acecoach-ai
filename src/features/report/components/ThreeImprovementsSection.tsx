...
export default function ThreeImprovementsSection({ report }: { report: AnalysisReport }) {
  ...
  return (
    <section id="improvements" className="scroll-mt-24 rounded-[2rem] border border-white/10 bg-slate-900/72 p-6 lg:p-8">
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-3">{view.improvements.length === 0 ? <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-5 text-sm leading-7 text-slate-300">Athlentra found one reliable priority in this recording. It is shown once in the Coach Summary rather than repeated here.</div> : null}
