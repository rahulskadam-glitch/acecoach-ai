    ...
    <div className="grid min-h-[68vh] gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <section className="flex min-h-[68vh] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="border-t border-slate-200 bg-white p-4 sm:p-6">
          ...
          <form onSubmit={(event) => { event.preventDefault(); void submit(); }} className="flex items-end gap-3"><label className="sr-only" htmlFor="coaching-question">Ask Athlentra</label><textarea id="coaching-question" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} rows={2} maxLength={1200} placeholder="Ask about the correction, cue, drill, or reassessment…" className="max-h-40 min-h-14 flex-1 resize-y rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm leading-6 outline-none focus:border-blue-700 focus:ring-2 focus:ring-blue-100" /><button type="submit" disabled={!draft.trim() || sending} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[#1b4332] text-white hover:bg-[#2d6a4f] disabled:bg-slate-300" aria-label="Send coaching question"><Send className="h-4 w-4" /></button></form>
